...
LogDistributerAnalyzer_JaccardSimilarity_qgramExact::~LogDistributerAnalyzer_JaccardSimilarity_qgramExact()
{
...
int LogDistributerAnalyzer_JaccardSimilarity_qgramExact::getBucket(const std::string& input)
{
    int best_index = -1;
    int index = 0;
...
int LogDistributerAnalyzer_JaccardSimilarity_qgramExact::getBucket(const char* str)
{
...
*/
float LogDistributerAnalyzer_JaccardSimilarity_qgramExact::averageJaccardInColumn(const list< set<string>* >* input, const set<string>* compare)
{
    float average;
    float best;
...
float LogDistributerAnalyzer_JaccardSimilarity_qgramExact::bestJaccardInColumn(const list< set<string>* >* input, const set<string>* compare)
{
    float average;
    float best;
...
void LogDistributerAnalyzer_JaccardSimilarity_qgramExact::JaccardStatsInColumn(const std::list< std::set<std::string>* >* input,
...
void LogDistributerAnalyzer_JaccardSimilarity_qgramExact::makeParsedSet(const std::string& input, set<std::string>* output)
{
    output->clear();

    unsigned startidx = 0;
    unsigned endidx = input.length() - m_qgramLength;

    while(startidx < endidx)
    {
        output->insert(input.substr(startidx, m_qgramLength));
        startidx++;
    }
}
